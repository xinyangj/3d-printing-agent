...
from printing_agent.domain import ModelArtifact, canonical_digest, utc_now
...
class SubmissionDriver(Protocol):
    ...
    async def validate(self, profile: PrinterProfileRevision) -> None: ...
    ...
    async def launch(self, handoff: SubmissionHandoff) -> SubmissionHandoff: ...
...
class SubmissionRegistry:
    def __init__(self) -> None:
        self._drivers: dict[str, SubmissionDriver] = {}

    def register(self, driver: SubmissionDriver) -> None:
        if driver.id in self._drivers:
            raise ConflictError(
                f"Submission driver '{driver.id}' is already registered"
            )
        self._drivers[driver.id] = driver

    def get(self, driver_id: str) -> SubmissionDriver:
        try:
            return self._drivers[driver_id]
        except KeyError as exc:
            raise NotFoundError(
                f"Submission driver '{driver_id}' is not registered"
            ) from exc
...
class BambuConnectCloudDriver:
    id = "bambu_connect_cloud"
    ...
    async def validate(self, profile: PrinterProfileRevision) -> None:
        if profile.spec.submission.mode != "bambu_connect_cloud":
            raise ValidationError("Printer profile does not use Bambu Connect cloud")
        if os.name != "nt":
            raise ExternalServiceError(
                "Bambu Connect URL handoff is currently supported only on Windows"
            )
        if self.require_registration and not await asyncio.to_thread(
            self._scheme_registered
        ):
            raise ExternalServiceError(
                "Bambu Connect is not installed or its URL scheme is not registered"
            )
    ...
    async def prepare_handoff(
        ...
        path = await asyncio.to_thread(lambda: Path(sliced.path).resolve())
        if (
            not await asyncio.to_thread(path.is_file)
            or path.suffixes[-2:] != [".gcode", ".3mf"]
        ):
            raise ValidationError("Submission handoff requires an immutable .gcode.3mf")
        if self.allowed_root not in path.parents:
            raise PolicyViolationError(
                "Sliced artifact is outside the immutable slice root"
            )
        self._validate_gcode_3mf(path)
        if sha256_file(path) != sliced.digest:
            raise ConflictError("Sliced artifact digest changed before handoff")
        safe_name = re.sub(r"[^A-Za-z0-9._ -]+", "_", path.stem).strip()[:100]
        query = urlencode(
            {
                "path": str(path),
                "name": f"{workflow_id[:8]}-{safe_name}",
                "version": "1.0.0",
            }
        )
        return SubmissionHandoff(
            workflow_id=workflow_id,
            slice_job_id=sliced.slice_job_id,
            sliced_artifact_digest=sliced.digest,
            submission_driver_id=self.id,
            status=HandoffStatus.READY,
            local_path=str(path),
            launch_uri=f"{profile.spec.submission.uri_scheme}://import-file?{query}",
        )

    async def launch(self, handoff: SubmissionHandoff) -> SubmissionHandoff:
        if handoff.launch_uri is None:
            raise ValidationError("Submission handoff has no launch URI")
        if handoff.local_path is None:
            raise ValidationError("Submission handoff has no local sliced file")
        if not handoff.launch_uri.startswith("bambu-connect://import-file?"):
            raise PolicyViolationError("Unsupported Bambu Connect launch URI")
        path = await asyncio.to_thread(lambda: Path(handoff.local_path).resolve())
        if self.allowed_root not in path.parents:
            raise PolicyViolationError(
                "Sliced artifact is outside the immutable slice root"
            )
        self._validate_gcode_3mf(path)
        if sha256_file(path) != handoff.sliced_artifact_digest:
            raise ConflictError("Sliced artifact digest changed before launch")
        try:
            await asyncio.to_thread(os.startfile, handoff.launch_uri)  # type: ignore[attr-defined]
        except OSError as exc:
            raise ExternalServiceError(
                "Bambu Connect could not be launched; install and register it first"
            ) from exc
        now = utc_now()
        return handoff.model_copy(
            update={
                ...
                "updated_at": now,
                "message": (
                    "Bambu Connect opened. Confirm the intended H2D and submit the job "
                    "inside Bambu Connect."
                ),
            }
        )
...
class SimulatorSubmissionDriver:
    id = "simulator"

    async def validate(self, profile: PrinterProfileRevision) -> None:
        if profile.spec.submission.mode != "simulator":
            raise ValidationError("Printer profile does not use simulator submission")
    ...
    async def launch(self, handoff: SubmissionHandoff) -> SubmissionHandoff:
        ...
        return handoff.model_copy(
            update={
                "status": HandoffStatus.USER_CONFIRMED_SUBMITTED,
                "launched_at": now,
                "confirmed_at": now,
                "confirmed_by": "simulator",
                "updated_at": now,
            }
        )
