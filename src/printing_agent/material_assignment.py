...
class MaterialAssignmentService:
    ...
    async def propose(
        ...
        *,
        ...
        return MaterialAssignment(
            ...
            cloud_snapshot_id=cloud_snapshot_id,
            cloud_snapshot_digest=cloud_snapshot_digest,
    ...
    def apply_user_overrides(
        ...
        spool_overrides: dict[str, str],
    ) -> MaterialAssignment:
        request_ids = {item.part_id for item in assignment.requests}
        if not set(spool_overrides).issubset(request_ids):
            raise ValidationError("Material overrides reference unknown parts")
        selected: list[PartMaterialAssignment] = []
        current = {
            item.part_id: item for item in assignment.assignments
        }
        for request in assignment.requests:
            part_id = request.part_id
            existing = current[part_id]
            spool_id = spool_overrides.get(part_id, existing.spool_id)
            candidates = assignment.candidate_options.get(part_id, [])
            candidate = next(
                (item for item in candidates if item.spool_id == spool_id),
                None,
            )
            if candidate is None:
                raise ValidationError(
                    f"Selected material tray is not compatible with part '{part_id}'"
                )
            toolhead_id = (
                existing.toolhead_id
                if existing.toolhead_id in candidate.toolhead_ids
                else sorted(candidate.toolhead_ids)[0]
            )
            selected.append(
                PartMaterialAssignment(
                    part_id=part_id,
                    ...
                    confidence=1,
                    rationale=(
                        "User selected this compatible observed cloud tray."
                        if spool_id != existing.spool_id
